<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Modifier l\'Exercice') }}</h2></x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    <form action="{{ route('exercices.update', $exercice) }}" method="POST">
                        ...
                        <div class="space-y-8">
                            <div>
                                <h2 class="text-lg font-medium">Détails de l'Exercice</h2>
                                <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                    <div class="sm:col-span-4">
                                        ...
                                        <select id="id_copropriete" name="id_copropriete" class="mt-2 block w-full rounded-md border-0 py-1.5 ring-1">
                                            @foreach ($coproprietes as $copropriete)
                                                <option value="{{ $copropriete->id_copropriete }}" @if(old('id_copropriete', $exercice->id_copropriete) == $copropriete->id_copropriete) selected @endif>{{ $copropriete->nom_copropriete }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="sm:col-span-3">
                                        ...
                                        <input type="date" name="date_debut" id="date_debut" value="{{ old('date_debut', $exercice->date_debut->format('Y-m-d')) }}" class="mt-2 block w-full rounded-md ring-1">
                                    ...
                                    <div class="sm:col-span-3">
                                        ...
                                        <input type="date" name="date_fin" id="date_fin" value="{{ old('date_fin', $exercice->date_fin->format('Y-m-d')) }}" class="mt-2 block w-full rounded-md ring-1">
                                    ...
                                    <div class="sm:col-span-3">
                                        ...
                                        <select id="statut" name="statut" class="mt-2 block w-full rounded-md ring-1">
                                            @foreach (App\Enums\StatutExercice::cases() as $statutEnum)
                                                <option value="{{ $statutEnum->value }}" @if(old('statut', $exercice->statut) == $statutEnum) selected @endif>
                                                    {{ $statutEnum->label() }}
                                                </option>
                                            @endforeach
                                        </select>
