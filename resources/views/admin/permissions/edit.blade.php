<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Modifier la Permission :') }} {{ $permission->getTranslated('nom_permission') }}</h2></x-slot>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <div class="mt-2"><input type="text" name="cle" id="cle" value="{{ old('cle', $permission->cle) }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300"></div>
                                ...
                                @foreach (config('languages.supported') as $locale => $language)
                                    <div>
                                        ...
                                        <div class="mt-2">
                                            <input type="text" name="nom_permission[{{ $locale }}]" id="nom_permission_{{ $locale }}" value="{{ old('nom_permission.' . $locale, $permission->nom_permission[$locale] ?? '') }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300">
                                        </div>
