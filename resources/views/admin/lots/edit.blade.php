    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <span class="text-base text-gray-500 font-normal ml-2">
                (Résidence: {{ $lot->residence->nom_residence ?? 'N/A' }})
            </span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    @if ($errors->any())<div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">Veuillez corriger les erreurs ci-dessous.</div>@endif
                    ...
                    <form action="{{ route('lots.update', $lot) }}" method="POST">
                        ...
                        <div class="space-y-8">
                            <div>
                                <h2 class="text-lg font-medium">Informations du Lot</h2>
                                <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                    <div class="sm:col-span-4">
                                        <label for="id_residence" class="block text-sm font-medium">Résidence</label>
                                        <select id="id_residence" name="id_residence" class="mt-2 block w-full rounded-md border-0 py-1.5 ring-1">
                                            @foreach ($residences as $residence)
                                                ...
                                                >
                                                    ...
                                                    @if(!$residence->statut) (Inactif) @endif
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="sm:col-span-3">
                                        ...
                                        <select id="statut" name="statut" class="mt-2 block w-full rounded-md border-0 py-1.5 ring-1">
                                            <option value="1" @if(old('statut', $lot->statut) == 1) selected @endif>Actif</option>
                                            <option value="0" @if(old('statut', $lot->statut) == 0) selected @endif>Inactif</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
