<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Add Lot') }}</h2></x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    <form action="{{ route('lots.store') }}" method="POST">
                        ...
                        <div class="space-y-8">
                            <div>
                                <h2 class="text-lg font-medium leading-6 text-gray-900">{{ __('Lot Informations') }}</h2>
                                <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                    <div class="sm:col-span-4">
                                        <label for="id_residence" class="block text-sm font-medium leading-6 text-gray-900">{{ __('Residence') }}</label>
                                        <div class="mt-2">
                                            <select id="id_residence" name="id_residence" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300">
                                                <option value="">-- {{ __('Select Residence') }} --</option>
                                                @foreach ($residences as $residence)
                                                    <option value="{{ $residence->id_residence }}" {{ old('id_residence') == $residence->id_residence ? 'selected' : '' }}>{{ $residence->nom_residence }}</option>
                                                @endforeach
                                            </select>
                                        </div>
