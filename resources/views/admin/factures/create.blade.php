                            ...
                            <div class="space-y-8">
                                <div>
                                    ...
                                    <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                        ...
                                        <div class="sm:col-span-4">
                                            ...
                                            <select id="id_fournisseur" name="id_fournisseur" required class="mt-2 block w-full rounded-md ring-1">
                                                ...
                                                @foreach($fournisseurs as $fournisseur)
                                                    <option value="{{ $fournisseur->id_fournisseur }}" @if(old('id_fournisseur') == $fournisseur->id_fournisseur) selected @endif>{{ $fournisseur->nom }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="sm:col-span-4">
                                            ...
                                            <select id="id_copropriete" name="id_copropriete" x-model="selectedCoproprieteId" required class="mt-2 block w-full rounded-md ring-1">
                                                ...
                                                @foreach($coproprietes as $copropriete)
                                                    <option value="{{ $copropriete->id_copropriete }}" @if(old('id_copropriete') == $copropriete->id_copropriete) selected @endif>{{ $copropriete->nom_copropriete }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-span-full"><label for="objet">{{ __('Objet') }}</label><input type="text" name="objet" id="objet" value="{{ old('objet') }}" required class="mt-2 block w-full ring-1"></div>
                                        <div class="sm:col-span-3"><label for="numero_facture">{{ __('N° Facture') }}</label><input type="text" name="numero_facture" id="numero_facture" value="{{ old('numero_facture') }}" class="mt-2 block w-full ring-1"></div>
                                        <div class="sm:col-span-3"><label for="montant_ttc">{{ __('Montant TTC') }}</label><input type="number" step="0.01" min="0" name="montant_ttc" id="montant_ttc" value="{{ old('montant_ttc') }}" required class="mt-2 block w-full ring-1"></div>
                                        <div class="sm:col-span-3"><label for="date_emission">{{ __('Date d\'émission') }}</label><input type="date" name="date_emission" id="date_emission" value="{{ old('date_emission') }}" required class="mt-2 block w-full ring-1"></div>
                                        <div class="sm:col-span-3"><label for="date_echeance">{{ __('Date d\'échéance') }}</label><input type="date" name="date_echeance" id="date_echeance" value="{{ old('date_echeance') }}" class="mt-2 block w-full ring-1"></div>
                                    </div>
                                </div>
                            </div>
            ...
            Alpine.data('factureForm', () => ({
                ...
                updateExercices(coproId, isInitialLoad = false) {
                    ...
                    if (selectedCopro && selectedCopro.exercices) {
                        console.log('Exercices trouvés dans la copro:', selectedCopro.exercices);
